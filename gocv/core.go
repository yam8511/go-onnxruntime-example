...
func DecodeOutput_Segment(img, output0, output1 Mat, origin_width, origin_height int) unsafe.Pointer {
	return C.decode_output_segment(img.p, output0.p, output1.p, C.int(origin_width), C.int(origin_height))
}
...
func newRange(rng C.Range) Range {
	return Range{rng: rng}
}

func NewRange() Range {
	return newRange(C.Range_New())
}

func NewRangeFrom(start, end int) Range {
	return newRange(C.Range_NewFrom(C.int(start), C.int(end)))
}

func NewRangeFromAll() Range {
	return newRange(C.Range_NewFromAll())
}

func Range_Size(rng Range) int {
	return int(C.Range_Size(rng.rng))
}

func Range_Empty(rng Range) bool {
	return bool(C.Range_Empty(rng.rng))
}

func Range_Start(rng Range) int {
	return int(C.Range_Start(rng.rng))
}

func Range_End(rng Range) int {
	return int(C.Range_End(rng.rng))
}
...
func NewMatWithSizeAndPtr(sizes []int, mt MatType, ptr unsafe.Pointer) Mat {
	...
	return newMat(C.Mat_NewWithSizesAndPtr(sizesIntVector, C.int(mt), ptr))
}
...
func (m *Mat) Negative() Mat {
	return newMat(C.Mat_Negative(m.p))
}

func (m *Mat) OpAdd_Float(n float32) Mat {
	return newMat(C.Mat_OpAdd_Float(m.p, C.float(n)))
}

func (m *Mat) OpGreat_Float(n float32) Mat {
	return newMat(C.Mat_OpGreat_Float(m.p, C.float(n)))
}

func (m *Mat) OpDivByNum_Float(n float32) Mat {
	return newMat(C.Mat_OpDivByNum_Float(m.p, C.float(n)))
}
...
func (m *Mat) Ranges(rngs RangeVector) Mat {
	return newMat(C.Mat_Ranges(m.p, rngs.p))
}
...
func (m *Mat) ReshapeWithSize(cn int, sizes []int) Mat {
	sizesArray := make([]C.int, len(sizes))
	for i, s := range sizes {
		sizesArray[i] = C.int(s)
	}
	...
	return newMat(C.Mat_ReshapeWithSizes(m.p, C.int(cn), sizesIntVector))
}
...
func (m *Mat) SetToWithMat(s Scalar, mask Mat) {
	sVal := C.struct_Scalar{
		val1: C.double(s.Val1),
		val2: C.double(s.Val2),
		val3: C.double(s.Val3),
	...
	C.Mat_SetToWithMat(m.p, sVal, mask.p)
}
...
// NewRangeVector returns a new empty RangeVector.
func NewRangeVector() RangeVector {
	return RangeVector{p: C.RangeVector_New()}
}

// Append appends an image.Point at end of the RangeVector.
func (pv RangeVector) Append(rng Range) {
	C.RangeVector_Append(pv.p, rng.rng)
	return
}

func (pv RangeVector) Size() int {
	return int(C.RangeVector_Size(pv.p))
}
