...
func (sess *Session_OD) run_model(input []float32) (*gocv.Mat, error) {
	...
	ptr, err := outputTensor.GetTensorMutableData()
	if err != nil {
		return nil, err
	}
	sizes := outputTensor.GetShape().Sizes()
	mat := gocv.NewMatWithSizeAndPtr([]int{sizes[1], sizes[2]}, gocv.MatTypeCV32F, ptr)
	return &mat, nil
}

func (sess *Session_OD) process_output(output *gocv.Mat, threshold float32) (
