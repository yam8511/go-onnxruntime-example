...
// NewMatWithSizesFromPtr returns a new multidimensional Mat with a specific size and type.
func NewMatWithSizesFromPtr(sizes []int, mt MatType, ptr unsafe.Pointer) Mat {
	sizesArray := make([]C.int, len(sizes))
	for i, s := range sizes {
		sizesArray[i] = C.int(s)
	}
	...
	return newMat(C.Mat_NewWithSizesFromPtr(sizesIntVector, C.int(mt), ptr))
}
